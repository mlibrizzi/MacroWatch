...
import { fetchDaily, fetchAuctions, fetchWeekly, fetchMonthly, fetchQuarterly, fetchIntel, fetchHistory } from './fetchers.js';
...
:root{
  --bg:#06090d;--s1:#0b1018;--s2:#111820;--s3:#182030;
  --b1:#1a2535;--b2:#243040;--b3:#304050;
  --t1:#e8f0f8;--t2:#8aa0b8;--t3:#4a6080;
  --acc:#00e5c0;--acc2:#0096ff;--acc3:#ffd060;
  --up:#00e5c0;--dn:#ff3e5a;--warn:#ffd060;--neu:#8aa0b8;
  --red:#ff3e5a;--amber:#ffd060;--green:#00e5c0;--blue:#0096ff;
  --gold:#fbbf24;
...
.hdr-date{font-family:var(--mono);font-size:9px;color:var(--t3);letter-spacing:1px;text-align:right;line-height:1.5}
.refbtn{background:none;border:1px solid var(--b2);color:var(--acc);padding:5px 10px;border-radius:var(--r);font-family:var(--mono);font-size:10px;cursor:pointer;transition:all .15s}
.refbtn:hover{background:rgba(0,229,192,.08);border-color:var(--acc)}
.refbtn:disabled{opacity:.35;cursor:not-allowed}
...
.lbl{font-size:12px;font-weight:500;color:var(--t1)}
.sub{font-family:var(--mono);font-size:9px;color:var(--t3);margin-top:1px}
...
.sdot.rd{background:var(--red);box-shadow:0 0 5px var(--red)}
.sdot.am{background:var(--amber);box-shadow:0 0 5px var(--amber)}
          ...
          <table className="rates-tbl">
            <tbody>
              {[['Fed Broad USD','Broad USD vs 26 currencies (not ICE DXY)', fx?.dxy], ['EUR/USD','Euro', fx?.eurusd], ['USD/JPY','Yen', fx?.jpyusd]].map(([lbl,sub,v]) => v ? (
                <tr key={lbl}><td><div className="lbl" style={{fontSize:12,display:"flex",alignItems:"center",gap:"2px"}}>{lbl}{lbl==='Fed Broad USD'&&<InfoBtn label="Fed Broad USD"/>}</div><div className="sub">{sub}</div></td>
          ...
          <div className="card" key={i}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 6 }}>
              {[
                ...
                ['INDIRECT', a.indirect_pct != null ? `${a.indirect_pct.toFixed(1)}%` : 'N/A', zoneColor(a.indirect_pct, IND_THRESHOLDS),        a.indirect_avg != null ? `avg ${a.indirect_avg.toFixed(1)}%` : ''],
                ['DEALER',   a.dealer_pct != null ? `${a.dealer_pct.toFixed(1)}%` : 'N/A',   zoneColor(a.dealer_pct,   DLR_THRESHOLDS,  true), a.dealer_avg != null ? `avg ${a.dealer_avg.toFixed(1)}%` : ''],
                ['TAIL',     a.tail_bp != null ? `${a.tail_bp.toFixed(1)}bp` : 'N/A',     zoneColor(a.tail_bp,      TAIL_THRESHOLDS, true), a.tail_avg_bp != null ? `avg ${a.tail_avg_bp.toFixed(1)}bp` : ''],
        ...
        <Sect title="TREASURY YIELDS 2Y / 10Y / 30Y" infoKey="Treasury Yields History" note="All 3 yields shown 4 years. 2Y above 10Y from 2022-2024 = inversion. Normal spread = +80bp.">
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={d.yieldCurve} margin={{top:4,right:8,left:-20,bottom:0}}>
              ...
              <Line type="monotone" dataKey="t10y" stroke="var(--acc2)" dot={false} strokeWidth={1.5} name="10Y"/>
              <Line type="monotone" dataKey="t30y" stroke="var(--acc3)" dot={false} strokeWidth={1.5} name="30Y"/>
            </LineChart>
          </ResponsiveContainer>
          <div style={{display:"flex",gap:"12px",padding:"4px 8px",fontSize:"9px"}}>
            <span style={{color:"var(--acc)"}}>2Y</span>
            <span style={{color:"var(--acc2)"}}>10Y</span>
            <span style={{color:"var(--acc3)"}}>30Y</span>
          </div>
        </Sect>
      )}
...
const TABS = [
  ...
  { id: 'history', label: 'TRENDS', loader: fetchHistory },
