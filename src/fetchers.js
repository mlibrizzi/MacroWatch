...
export async function fetchWeekly() {
  try {
    ...
    const fedPolicy = await callClaude('Fed rate as of '+TODAY+'. Next FOMC June 16-17 2026. April 29 held 4 dissents. Return JSON: current_rate string, next_meeting string, market_cut_prob_jun_pct number, powell_status string, qt_status string, fomc_dissents number, fomc_note string.', 'Return only raw JSON. Start with {')rt { callClaude } from './api.js';
...
export async function fetchAuctions() {
  const data = await callClaude('Return JSON with recent Treasury auction results as of '+TODAY+'. Fields: recent array with term/date/size_bn/bid_to_cover/indirect_pct/dealer_pct/high_yield/tail_bp/status/note. Upcoming array with term/date/size_bn. macro_note string.', 'Return only raw JSON. Start with {')rt { callClaude } from './api.js';
...
export async function fetchAuctions() {
  const data = await callClaude(`Return JSON with the 5 most recent US Treasury auction results across 2Y, 5Y, 7Y, 10Y and 30Y maturities, and upcoming auctions, as of ${TODAY}.
{
  "recent": [
    {
      "term": "2-Year Note",
  ...
  return {
    ...data,
    delay: 'AI estimate — verify same-day at treasurydirect.gov/auctions'
  };
}

export async function fetchMonthly() {
  const data = await callClaude('Return JSON with TIC foreign holdings and basis trade data as of '+TODAY+'. Fields: tic object with report_month/total_foreign_bn/foreign_share_pct/china_net_since_2021_bn/top_holders array. basis_trade object with estimated_size_tn/stress_level/note/sofr_treasury_spread_bp.', 'Return only raw JSON. Start with {')rt { callClaude } from './api.js';
...
export async function fetchQuarterly() {
  const data = await callClaude('As of '+TODAY+' Fed 3.50-3.75%. GDP Q1 2026 +2.0%. Return JSON with earnings array and gov_reports array. Upcoming: AMZN May 6, GOOGL May 8, META May 12, NVDA May 20.', 'Return only raw JSON. Start with {')rt { callClaude } from './api.js';
...
export async function fetchIntel(userQuestion) {
  ...
  if (userQuestion) {
    return callClaude('Answer with JSON: answer, key_points, data_sources. Question: '+userQuestion, sys);
  }
  return callClaude('Today '+TODAY+'. Fed 3.50-3.75pct. Gold 4615. VIX 17. SP500 7200. 10Y 4.38pct. CPI 3.3pct. GDP Q1 2.0pct. Deficit 2T. Debt/GDP 122pct. 4 FOMC dissents April 29 2026. Return JSON: thesis, regime, regime_confidence, alerts array with level/title/detail/category, key_risks array with risk/probability/horizon, key_watches array with indicator/why/threshold/source, dalio_lens, positioning with usd/gold/long_bonds/equities/rationale.', sys);
}
